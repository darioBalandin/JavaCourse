...
public class Student {
    ...
    private static Course[] getRandomCourses(Course... courses) {
        ...
        var newList = new ArrayList<Course>();

        for (var el : courses) {
            if (random.nextBoolean()) newList.add(el);
        }
        if (newList.size() < 1) {
            var random = Arrays.stream(courses).findAny();
            random.ifPresent(newList::add);
        }
        return newList.toArray(Course[]::new);
    }
    ...
    public static Student getRandomStudent(Course... courses) {
        ...
        var randomCourses = getRandomCourses(courses);
